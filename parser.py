...
def download_book(url_book,id):    
    ...
    filename = f"{id+1}-я книга. {parsing_text(url_book)}.txt"
...
if __name__ == '__main__':
    ...
    for id in range(10):
        url_book = urls[id]
        url_img = parsing_image(url_book)
        book_info = parsing_text(url_book).split(' -- ')
        comments = parsing_comments(url_book)
        genres = parsing_genres(url_book)
